...
void Km_Draw::createSleepers_Stage2(int x_start, int x_stop, int index_start_sleeper, int coun_mini_sleeper, int bias_cycle){
    ...
    bool check_dop = false;

    int count = coun_1996_sleepers_on_km;


    float coun = (x_start - x_stop) / one_km_width;

    int count_cycle = coun;

    int count_dop_sleepers = (coun - count_cycle) * coun_1996_sleepers_on_km;


    for(int i = 0; i < count_cycle + 1; i++){ // 11 - Здесь количество километров на графике
        ...
        for(int k = 0; k < count; k++){ // 10 - Здесь количество тройных рисок на километр
            ...
            for(int j = index_start_sleeper; j < coun_mini_sleeper; j += bias_cycle){ // for(int j = 0; j < 3; j++){
                ...
                float x =  x_start -  new_space - ((j + 1) * 0.2 * m_pointer->coef);
            ...
            if(i == count_cycle && !check_dop){ // Если это последнее значение  //   if(i == coun_km){
...
void Km_Draw::createSleepers_Stage1(int x_start, int x_stop, int index_type){
    ...
    createSleepers_Stage2(x_start, x_stop, index_start_sleeper, coun_mini_sleeper, bias_cycle);
...
void Km_Draw::createHelpLine(int x_start, int x_stop){

    m_pointer->drawLine(QPointF(x_start, helpLine_y), QPointF(x_stop, helpLine_y), Pdf_Values::getPen(Pdf_Values::KmPen));

}

void Km_Draw::createEarthLine(int x_start, int x_stop){

    m_pointer->drawLine(QPointF(x_start, earthLine_y), QPointF(x_stop, earthLine_y), Pdf_Values::getPen(Pdf_Values::Km_Opacity));

}

int Km_Draw::getSpacing(QString text){

    QList<float> size = m_pointer->getSize(text);

    return m_pointer->spacingMeasure * 1.5 + size[0];
}
...
void Km_Draw::createKm_Block(float heightTitle, float leftX, float rightX, float heightKm){
  ...
  //  int picket_count = 11;
    ...
    for(int i = 0; i < coun_km; i++){
        ...
        // getSpacing(km_num)
        // km_x - m_pointer->spacingMeasure * 1.5 - size[0] / 2
        m_pointer->drawText(QPointF(km_x -   getSpacing(km_num), km_y - m_pointer->spacingMeasure / 2),  km_num);
   ...
   // createRiht(rightX - 4 * one_km_width, left_finish_x, false);
    ...
    m_pointer->drawText(QPointF(rightX - getSpacing(km_str) , km_y - m_pointer->spacingMeasure / 2), km_str);
    ...
    helpLine_y = km_y - size[1] - m_pointer->spacingMeasure * 0.2; // - m_pointer->spacingMeasure / 2;

    earthLine_y = helpLine_y + m_pointer->spacingMeasure; // / 2;


    createHelpLine(rightX, rightX - diff / 4);

    createHelpLine(rightX - diff / 2, left_finish_x);



    createEarthLine(rightX, rightX - diff / 6);

    createEarthLine(rightX - diff / 3, left_finish_x);


    //createSleepers(y_TopRail, y_BottomRail, rightX, Before_1996);


    createSleepers_Stage1(rightX, rightX - 2800,  Wood);

    createSleepers_Stage1(rightX - 2800, left_finish_x,  Reinforced_Concrete);





   // m_pointer->drawLine(QPointF(left_finish_x, y_TopRail), QPointF(left_finish_x, y_TopRail + 700), Pdf_Values::getPen(Pdf_Values::Test));
